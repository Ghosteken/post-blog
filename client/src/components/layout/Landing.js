          ...
          <tbody>
            {posts && posts.length > 0 ? (
              posts.map((post, index) => (
                <tr key={index}>
                  <td>{post.title}</td>
                  <td>{post.model ? ReactHtmlParser(post.model) : 'No content'}</td>
                  <td>{post.date}</td>
                  <td>
                    <button onClick={() => gotoPost(post._id)} className="btn btn-secondary">View</button>
                  </td>
                  <td>
                    <button onClick={() => onDeletePost(post._id)} className="btn btn-secondary">Delete</button>
                  </td>
                </tr>
              ))
            ) : (
